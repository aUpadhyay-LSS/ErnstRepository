  ...
  if(isset($_POST['state']))
   {
		...
		$EstimatedValue=mysql_real_escape_string($_POST['purchase_price']);
		$MortgageAmount=mysql_real_escape_string($_POST['loan_amount']);
		//echo $ernst_state;
		if($ernst_state!='') 
		{
			...
			if (mysql_num_rows($query_result)>0)
			{
				...
				$PageREC = $row['PageREC'];
				GenerateXML($PageREC,$EstimatedValue,$MortgageAmount);
				//echo $PageREC;
			}
			else
			{
